...
def add_flagscale_args(parser):
    _add_emu_args(parser)

    return parser


def _add_emu_args(parser):

    group = parser.add_argument_group(title='Emu-MultiModal-MoE')

    group.add_argument('--language-vocab-size', type=int, default=None,
                       help='Size of language vocab before EOD or padding.')
    group.add_argument("--multimodal-freeze-language-parameters", action='store_true')
