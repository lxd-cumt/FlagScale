...
class LanguageModelEmbedding(MegatronModule):
    ...
    def __init__(
        ...
        self.vision_vocab_size: int = vision_vocab_size
        ...
        # NOTE(zhaoyinglia): for emu3 must be False
        self.reduce_scatter_embeddings = False

        # Word embeddings (parallel).
        self.word_embeddings = tensor_parallel.VocabParallelEmbedding(
            num_embeddings=self.language_vocab_size,
        ...
        # Vision embedding (parallel).
        self.vision_embeddings = tensor_parallel.VocabParallelEmbedding(
            num_embeddings=self.vision_vocab_size,
            embedding_dim=self.config.hidden_size,
            init_method=self.config.init_method,
            reduce_scatter_embeddings=self.reduce_scatter_embeddings,
            config=self.config,
    ...
    def zero_parameters(self):
        ...
        self.vision_embeddings.weight.data.fill_(0)
        self.vision_embeddings.weight.shared = True
    ...
    def forward(self, input_ids: Tensor, position_ids: Tensor, tokentype_ids: int = None, multimodal_mask: Tensor = None) -> Tensor:
        ...
        """
        split input_ids to language_ids and vision_ids =======================
        """
        args = get_args()
        b, s = input_ids.shape
        h = self.config.hidden_size

        language_ids = input_ids[multimodal_mask]
        vision_ids = input_ids[~multimodal_mask] - args.language_vocab_size

        language_embeddings = self.word_embeddings(language_ids)
        vision_embeddings = self.vision_embeddings(vision_ids)
        ...
        embeddings_mask = multimodal_mask[:, :, None].expand_as(word_embeddings)
        word_embeddings[embeddings_mask] = language_embeddings.flatten()
        word_embeddings[~embeddings_mask] = vision_embeddings.flatten()
